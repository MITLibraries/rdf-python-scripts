...
parser.add_argument('-r', '--rdfFileName', help='the RDF file to which triples \
will be added (include the extension). optional - if not provided, the script \
will ask for input')
parser.add_argument('-f', '--fileName', help='the CSV file of new triples \
(including \'.csv\'). optional - if not provided, the script will ask for \
input')
...
if args.rdfFileName:
    ...
    rdfFileName = input('Enter the RDF file to which triples will be added \
...
g.serialize(format='n3', destination=open(rdfFileName[:rdfFileName.index('.')]
            + 'Backup' + timeStamp + '.n3', 'w'))

# creating dict of existing labels for comparison
q = prepareQuery('SELECT ?s ?o WHERE { ?s skos:prefLabel ?o }',
                 initNs={'skos': SKOS})
existingLabels = {}
uriNums = []
results = g.query(q)
for row in results:
    uriNums.append(str(row[0]).replace('[Base URI]', ''))
...
# create log file
f = csv.writer(open(os.path.join('triplesAdded',
               rdfFileName[:rdfFileName.index('.')] + 'TriplesAdded'
               + timeStamp + '.csv'), 'w'))
...
with open(fileName) as csvfile:
    ...
    for row in reader:
        ...
        try:
            ...
            uriNum += 1
            subjectUri = '[Base URI]' + str(uriNum)
...
f = csv.writer(open(os.path.join('findAndReplace',
               rdfFileName[:rdfFileName.index('.')] + 'FindAndReplace'
               + timeStamp + '.csv'), 'w'))
f.writerow(['replacedValue'] + ['replacementValue'])
q = prepareQuery('SELECT ?altLabel ?prefLabel WHERE { ?s skos:prefLabel \
?prefLabel. ?s skos:altLabel ?altLabel }', initNs={'skos': SKOS})
results = g.query(q)
for row in results:
    f.writerow([row[0]] + [row[1]])

# extract prefLabels to csv
f = csv.writer(open(os.path.join('prefLabels', 'prefLabels' + timeStamp
               + '.csv'), 'w'))
f.writerow(['uri'] + ['prefLabel'])
q = prepareQuery('SELECT ?s ?prefLabel WHERE { ?s skos:prefLabel ?prefLabel }',
                 initNs={'skos': SKOS})
results = g.query(q)
for row in results:
    f.writerow([row[0]] + [row[1]])
...
f = csv.writer(open(os.path.join('allTriples', 'allTriples' + timeStamp
               + '.csv'), 'w'))
